...
We used the IBM Telco Customer Churn dataset, which contains 7,043 customer records. Each row represents one customer and includes account details, services, contract type, payment method, tenure, monthly charges, total charges, and whether the customer churned. Our goal was to compare three interpretable models that could help a telecom company identify customers who may be at risk of leaving.

## Assumption Checks

| Model | Key Assumptions Checked | Evidence | Concern |
|---|---|---|---|
| Linear regression | Linearity, multicollinearity, homoscedasticity, normality of residuals, and influential outliers | Redundant service categories and highly related charge variables were removed, leaving every VIF below 3. The Breusch-Pagan and Jarque-Bera tests both had p-values near zero. The maximum Cook's Distance was 0.0021. | Homoscedasticity and normality failed, which is expected when OLS is used with a binary outcome. This makes standard errors, p-values, and confidence intervals less reliable. |
| Logistic regression | Binary outcome, independent observations, limited multicollinearity, sufficient sample size, no perfect separation, and linearity in the log-odds | The target has two classes, each customer appears once, no missing values remained after cleaning, correlated charge variables were removed, the model converged, and no obvious separation candidates were found. The remaining continuous predictor, tenure, was checked against the log-odds of churn. | Removing MonthlyCharges and TotalCharges reduced multicollinearity but also removed potentially useful information. The model does not include nonlinear effects or interactions unless they are added directly. |
...
|---|---|---|---|
| Linear regression | Test R² = 0.263, RMSE = 0.379, and accuracy = 79.8% at a 0.5 threshold | Coefficients can be explained directly as changes in predicted churn probability. | About 17% of test predictions fall outside the valid 0 to 1 probability range, so the raw predictions should not be used as customer risk scores. |
| Logistic regression | Test accuracy = 82.1% | Coefficients show how each feature changes the log-odds of churn, and exponentiated coefficients can be presented as odds ratios. It also produces probabilities between 0 and 1. | Log-odds are less intuitive than percentage-point changes, and the current model assumes the effect of tenure is linear on the log-odds scale. |
